...
def get_partial_score(partial_paths, cfg):
    scores = []
    pbar = tqdm(partial_paths)
    for partial_path in pbar:
        ...
        full_path = os.path.join(cfg['data_path'], file_type, gt_file_name,
                                 gt_file_name + '_normalized.obj')
        ...
        scores.append(score)
        pbar.set_description(f"{file_type}-{gt_file_name}-{score}")
...
if cfg['action'] == "texture":
    train_all = glob(
        os.path.join(
            cfg['data_path'], 'train',
            cfg['preprocessing']['voxelized_colored_pointcloud_sampling']['input_files_regex'][3:]))
    ...
    predict = glob(
        os.path.join(
            cfg['data_path'], 'test-codalab-partial',
            cfg['preprocessing']['voxelized_colored_pointcloud_sampling']['input_files_regex'][3:]))
